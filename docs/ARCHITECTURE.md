...
| Meter data | GET + `search` for `dlp`, `bp`, `ls`, `events`, `alarms`; `GET ip/latest`; POST ingestion for each (for MDMS); alarm `acknowledge`/`resolve`; `dlp-completeness`; energy validation; Dashboard analytics (`analytics/meter-communication`: installed meters by last data received, from `Meters.LastCommunicatedAt` which ingestion keeps forward-only; `analytics/wallet-distribution`); Tariff & Parameters (`tariffs`, `tariffs/book-check`, `tariff-parameters`: the FY 2026-27 catalogue with schedule code, voltage, energy unit and fixed charge basis, compared with the book), the Calculation Workbench (`calculation-workbench/simulate`, one day's bill from `DailyBillCalculator`, the same code the daily run debits from), `consumers/{account}/daily-bills`; Live RC DC Status report (`reports/live-rc-dc`: per day, DC triggered/pending/success, RC raised against that day's DCs, and those consumers' recharges by meter-credit outcome; eligible DC is live, today only); billing holds (`billing-holds`, `billing-holds/search`, `billing-holds/summary`, `clear`, `clear-bulk`) |
...
- No shared-transformer-capacity billing basis for TMC (tariff book §5.2: MePDCL using a consumer's spare transformer capacity for other consumers) — `Consumer.SetBillingFacts` and the daily run only support the exclusive-use basis (§5.1).
