...
public class PasswordResetService
{
    ...
    public PasswordResetService(PrepaidEngineDbContext db, UserStore users, IEmailSender email, LoginThrottle throttle, ILogger<PasswordResetService> logger)
    {
        _db = db;
        _users = users;
        _email = email;
    ...
    {
        var user = _users.Find(loginId);
        if (user is null || string.IsNullOrWhiteSpace(user.Email))
        {
            if (user is not null) _logger.LogWarning("Password reset requested for {User}, but no e-mail address is registered for that login id.", user.Username);
            return;
        }

        var recent = await _db.PasswordResetRequests.AsNoTracking()
            .Where(r => r.LoginId == user.Username && r.RequestedAt > nowUtc.AddHours(-1))
        ...
        {
            _logger.LogWarning("Password reset code for {User} not sent: asked for too often.", user.Username);
            return;
        }

        foreach (var open in await _db.PasswordResetRequests.Where(r => r.LoginId == user.Username && r.UsedAt == null && r.SupersededAt == null).ToListAsync(ct))
            open.Supersede(nowUtc);

        var code = RandomNumberGenerator.GetInt32(0, 1_000_000).ToString("D6");
        var salt = RandomNumberGenerator.GetBytes(16);
        _db.PasswordResetRequests.Add(new PasswordResetRequest(Guid.NewGuid(), user.Username, Hash(code, salt), Convert.ToBase64String(salt), nowUtc, CodeLifetime, ip));
        _db.AuditEntries.Add(new AuditEntry(Guid.NewGuid(), "Auth", user.Username, "PASSWORD_RESET_REQUESTED", user.Username, nowUtc, details: "A reset code was e-mailed to the registered address."));
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                _logger.LogError(ex, "Could not e-mail the password reset code for {User}.", user.Username);
            }
        });
    }
    ...
    public async Task<ResetResult> ResetAsync(string? loginId, string? code, string? newPassword, DateTime nowUtc, CancellationToken ct = default)
    {
        ...
        var request = (await _db.PasswordResetRequests.Where(r => r.LoginId == user.Username).OrderByDescending(r => r.RequestedAt).Take(5).ToListAsync(ct))
        ...
        if (!CryptographicOperations.FixedTimeEquals(expected, actual))
        {
            ...
            _db.AuditEntries.Add(new AuditEntry(Guid.NewGuid(), "Auth", user.Username, "PASSWORD_RESET_FAILED", user.Username, nowUtc, details: $"Wrong code (attempt {request.Attempts} of {PasswordResetRequest.MaxAttempts})."));
            await _db.SaveChangesAsync(ct);
        ...
        var problems = PasswordPolicy.Problems(newPassword, user.Username);
        if (problems.Count > 0)
        ...
        var existing = await _db.UserPasswordOverrides.FirstOrDefaultAsync(o => o.LoginId == user.Username, ct);
        if (existing is null) _db.UserPasswordOverrides.Add(new UserPasswordOverride(user.Username, hash, nowUtc));
        else existing.Replace(hash, nowUtc);

        request.MarkUsed(nowUtc);
        foreach (var other in await _db.PasswordResetRequests.Where(r => r.LoginId == user.Username && r.UsedAt == null && r.SupersededAt == null && r.Id != request.Id).ToListAsync(ct))
            other.Supersede(nowUtc);
        _db.AuditEntries.Add(new AuditEntry(Guid.NewGuid(), "Auth", user.Username, "PASSWORD_RESET_COMPLETED", user.Username, nowUtc));
        await _db.SaveChangesAsync(ct);

        _throttle.RecordSuccess(user.Username); // a locked-out user who has just proved they own the account can sign in
